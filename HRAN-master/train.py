...
def main(mode, config, use_bokeh=False):
    ...
    if mode == "train":
        ...
        sentence_representations_list=encoder.apply(source_sentence, source_sentence_mask);#encoder source
        ...
        # for i in range(config['ctx_num']):
        #     tmp_rep=encoder.apply(context_sentences[i],context_sentence_masks[i]);#encoder the i context
        #     tmp_rep=tmp_rep.dimshuffle(['x',0,1,2]);
        #     #concat the source decoder output and context encoder output
