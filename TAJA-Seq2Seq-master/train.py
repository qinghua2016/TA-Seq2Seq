...
def main(mode, config, use_bokeh=False):
    ...
    if mode == "train":
        ...
        source_sentence = tensor.wmatrix('source')
        source_sentence_mask = tensor.fmatrix('source_mask')
        target_sentence = tensor.wmatrix('target')
        target_sentence_mask = tensor.fmatrix('target_mask')
        # sampling_input = tensor.wmatrix('input')
        source_topical_word=tensor.wmatrix('source_topical')
        source_topical_mask=tensor.fmatrix('source_topical_mask')
        ...
        for i, line in enumerate(test_stream.get_epoch_iterator()):
            ...
            seq = numpy.array(sutils._oov_to_unk(
                line[0], config['src_vocab_size'], unk_idx),'int16')
            # seq2 = line[1];

            seq2 = numpy.array(line[1], 'int16');
